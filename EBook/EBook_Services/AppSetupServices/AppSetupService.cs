...
namespace EBook_Services.AppSetupServices
{
    public class AppSetupService : IAppSetupService
    {
        ...
        public async Task<AccountSignInResDTO> SignIn(AccountSignInDTO signInDTO, GenrateTokenDelgate tokenGenrator, CancellationToken cancellationToken, bool IsPasswordRequired = true)
        {
            ...
            try
            {
                var data = await ado.GetUserLoginCredentials(signInDTO.UserID, cancellationToken);
                if (data == null)
                {
                ...
                if ((PasswordHash != null && PasswordHash.Equals(data.PasswordHash) && IsPasswordRequired) || IsPasswordRequired == false)
                {
                    ...
                    //else if (user_Info.IsApplicationAccessAllowed == false && StaticPublicObjects.ado.IsAllowedDomain() && user_Info.IsAPIAccessAllowed == true && user_Info.IsAdmin == false)
                    //{
                    //    res.ErrorMsg = "You Don't Have Rights In This Application3";
                    //    return res;
                    //}
                    //else if (user_Info.IsApplicationAccessAllowed == true && StaticPublicObjects.ado.IsAllowedDomain() == false && StaticPublicObjects.ado.IsSwaggerCall() == false && user_Info.IsAPIAccessAllowed == false && user_Info.IsAdmin == false)
                    //{
                    //    res.ErrorMsg = "You Don't Have Rights In This Application4";
                    //    return res;
                    //}
