...
namespace EBook_Services.GlobalService
{
    public class ADORepository : IADORepository
    {
        ...
        {
            //string ConnectionString = "Data Source=DESKTOP-L2UK5S8;Initial Catalog=MSPL_DB;Integrated Security=True;";

            //using (SqlConnection con = new SqlConnection(ConnectionString))
            //{
            //    string sqlQuery = "SELECT * FROM T_Chats for json path";
            ...
            //    while (rdr.Read())
            //    {
            //        var json = rdr["FirstName"].ToString();
            //    }
            //    con.Close();
            //}
            ...
            DataRow? dataRow = null;
            List<Dynamic_SP_Params> List_Dynamic_SP_Params = new List<Dynamic_SP_Params>() {
                new Dynamic_SP_Params {ParameterName = "CR_ID", Val = CR_ID},
                new Dynamic_SP_Params {ParameterName = "UserName", Val = "HAMMAS.KHAN"},
            };

            object? result = null;
            result = ExecuteSQL("SELECT * FROM T_Chats for json path", false, 0, ref List_Dynamic_SP_Params, false, AppEnum.Database_Name.EBook, true, "");
            if (result != null)
            {
    ...
    public class LogFile : ILogFile
    {
        ...
        public void ErrorLog(string LogEntryDateTime = "", string RequestURL = "", string ParameterDetail = "", string FunctionName = "", string SmallMessage = "", string Message = "")
        {
            try
            {
                ...
                //FInfo = new FileInfo(string.Format(@"{0}\ErrorLog\{1}.txt", AppDomain.CurrentDomain.BaseDirectory, FileName));
                FInfo = new FileInfo(string.Format(@"{0}\ErrorLog\{1}.txt", _env.ContentRootPath, FileName));
                if (!FInfo.Directory.Exists)
                    FInfo.Directory.Create();

                //File.AppendAllText(AppDomain.CurrentDomain.BaseDirectory + @"\ErrorLog\\" + FileName + ".txt", sb.ToString());
